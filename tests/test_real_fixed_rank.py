...
def test_inner(man, X):
    for i in range(10):
        ...
        V = X.V
        Pinv = X.Pinv
        inn1 = al[0]*trace(eta1.tU.T @ eta2.tU) +\
            (al[1]-al[0])*trace((eta1.tU.T@U) @ (U.T@eta2.tU)) +\
            gm[0]*trace(eta1.tV.T @ eta2.tV) +\
            (gm[1]-gm[0])*trace((eta1.tV.T@V) @ (V.T@eta2.tV)) +\
            bt*trace(Pinv@eta1.tP@Pinv@eta2.tP.T)
        assert(allclose(man.inner(X, eta1, eta2), inn1))
        eta2b = man.g(X, eta2)
        inn2 = man.base_inner_ambient(eta1, eta2b)
        inn3 = trace(eta1.tU @ eta2b.tU.T) + trace(eta1.tP @ eta2b.tP.T) +\
            trace(eta1.tV @ eta2b.tV.T)
        print(inn1, inn2, inn3)
        
        eta2a = man.g_inv(X, eta2)
        v1 = man.inner(X, eta1, eta2a)
        v2 = trace(eta1.tU @ eta2.tU.T) + trace(eta1.tP @ eta2.tP.T) +\
            trace(eta1.tV @ eta2.tV.T)
...
def make_N_mat(man, X):
    ...
    for i in range(man.dim):
        ...
        nmat[:, i] = man._vec(
            N(man, X,
              Ux[:dB].reshape(m-p, p),
              Ux[dB:dB+dC].reshape(n-p, p),
              Ux[dB+dC:].reshape(p, p)))
    return nmat
...
def testN(man, X):
    ...
    ee2 = man._unvec(nmat @ np.concatenate(
        [B.reshape(-1), C.reshape(-1), D.reshape(-1)]))
...
def Hz(man, X, omg):
    ...
    Piv = X.Pinv
    return al[1]*X.U.T@omg.tU + gm[1]*X.V.T@omg.tV +\
...
def test_J(man, X):
    ...
    print(Dmsolve - (1/(al[1]+gm[1]))*(asym(X.V.T@omg.tV - X.U.T@omg.tU)))
    Dpsolve = extended_lyapunov(1/bt, 1/(al[1]+gm[1]), X.P, X.P@Dp@X.P)
    ...
    def solveNTgN(X, Bo, Co, Do):
        ...
        Dp_ = extended_lyapunov(1/bt, 1/(al[1]+gm[1]), X.P, X.P@Dp@X.P)
    ...
    def testNTgN(man, X):
        ...
        out1 = NTgN @ np.concatenate(
            [B0.reshape(-1), C0.reshape(-1), D0.reshape(-1)])
...
def test_christ_flat():
    ...
    nabla_xi_va = dv + man.g_inv(
        S, super(RealFixedRank, man).christoffel_form(S, xi, vv))
...
def test_rhess_02():
    ...
    def eta_field(Sin):
        return man.proj(S, fr_ambient(
            mU1 @ (Sin.U - S.U) @ m2,
            mV1 @ (Sin.V - S.V) @ m2,
            sym((m_p @ (Sin.P - S.P).reshape(-1)).reshape(p, p)))) + eeta
