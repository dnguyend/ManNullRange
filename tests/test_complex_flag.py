...
from ManNullRange.manifolds.ComplexFlag import ComplexFlag
from ManNullRange.manifolds.tools import crandn
...
def test_inner(man, Y):
    for i in range(10):
        ...
        """
        alpha = man.alpha
        inn1 = alpha[0]*trace(eta1.T.conjugate() @ eta2) +\
            (alpha[1]-alpha[0])*trace((eta1.T.conjugate()@Y) @ (Y.T.conjugate()@eta2))
        assert(allclose(man.inner(Y, eta1, eta2), inn1))
        """
        eta2a = man.g_inv(Y, eta2)
        v1 = man.inner(Y, eta1, eta2a)
        v2 = trace(eta1 @ eta2.T.conjugate()).real
...
def test_J(man, Y):
    ...
    def diff_i_j(i, j):
        ...
        for r, s in ju:
            ...
            if r == s:
                asum += check_zero(
                    ju[r, r] - alpha[r-1, r]*Y[:, br:er].T.conjugate() @
                    U[:, br:er])
            else:
                bs, es = gdc[s]
...
def misc_test_cg(Y, b):
    ...
    Amat = np.eye(Y.shape[0]) - .5*Y @ Y.T.conjugate()
...
def test_rhess_02():
    ...
    def ehess_form(W, xi, eta):
        ...
        for rr in range(p):
            br, er = gidx[rr+1]
            ss += 2 * trace(UU[rr] @ xi[:, br:er] @
                            eta[:, br:er].T.conjugate()).real
        return ss
    ...
    xietaf = trace(df(Ynew) @ eta_field(Ynew).T.conjugate() -
                   df(Y) @ eta_field(Y).T.conjugate()).real / dlt
    # appy eta_func to f: should go to tr(m1 @ xxi @ m2 @ df(Y).T.conjugate())
    Dxietaf = trace((eta_field(Ynew) -
                     eta_field(Y)) @ df(Y).T.conjugate()).real/dlt
    ...
    #                              trace(ehvec@eta1.T.conjugate()).real)
    ...
    first = (e2 - e1)/dlt
    first1 = trace(df(Ynew) @ eta_proj(Ynew).T.conjugate() -
